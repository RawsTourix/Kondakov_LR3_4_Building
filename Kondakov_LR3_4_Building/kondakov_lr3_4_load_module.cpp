    ...
    if (file.is_open()) {
        ...
        if (building_params.size() % Building::SIZE == 0) {
            for (const string& param : building_params) {
                ...
                default:
                    cout << "LoadModule: ќшибка записи данных! ќбнаружен неизвестный параметр здани€." << endl << endl;
                    file.close();
                    return buildings;
                }
                param_id++;
            ...
            cout << "LoadModule: ќшибка: обнаружены лишние данные при чтении бинарного файла!" << endl << endl;
